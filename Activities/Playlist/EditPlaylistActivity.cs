                    ...
                    if (apiStatus.Equals(200))
                    {
                        if (respond is MessageObject result)
                        {
                            ...
                            RunOnUiThread(() =>
                            {
                                try
                                {
                                    var dataPlaylistFragment = HomeActivity.GetInstance()?.PlaylistFragment?.PlaylistAdapter;
                                    var list = dataPlaylistFragment?.PlaylistList;
                                    var dataPlaylist = list?.FirstOrDefault(a => a.Id == PlaylistObject?.Id);
                                    if (dataPlaylist != null)
                                    { 
                                        dataPlaylist.Name = NameEditText.Text;
                                        dataPlaylist.Privacy = Convert.ToInt32(Status);

                                        if (!string.IsNullOrEmpty(PathImage))
                                            dataPlaylist.ThumbnailReady = PathImage;
                                          
                                        int index = list.IndexOf(dataPlaylist);
                                        if (index >= 0)
                                            dataPlaylistFragment.NotifyItemChanged(index);
                                    }
