            ...
            try
            {
                if (Methods.CheckConnectivity())
                {
                    switch (SongsType)
                    {
                        ...
                        case "MyProfileStore":
                        {
                            if (GlobalContext?.ProfileFragment?.StoreAdapter?.SoundsList?.Count > 0)
                            {
                                MAdapter.SoundsList = GlobalContext?.ProfileFragment?.StoreAdapter?.SoundsList;
                                MAdapter.NotifyDataSetChanged();
                            } 
                            break;
                        }
                        case "MyProfileTopSongs":
                        {
                            if (GlobalContext?.ProfileFragment?.TopSongsAdapter?.SoundsList?.Count > 0)
                            {
                                MAdapter.SoundsList = GlobalContext?.ProfileFragment?.TopSongsAdapter?.SoundsList;
                                MAdapter.NotifyDataSetChanged();
                            } 
                            break;
                        }
                        case "MyProfileLatestSongs":
                        {
                            if (GlobalContext?.ProfileFragment?.LatestSongsAdapter?.SoundsList?.Count > 0)
                            {
                                MAdapter.SoundsList = GlobalContext?.ProfileFragment?.LatestSongsAdapter?.SoundsList;
                                MAdapter.NotifyDataSetChanged();
                            } 
                            break;
                        }
