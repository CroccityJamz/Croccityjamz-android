    ...
    public class EditSongActivity : AppCompatActivity, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                CurrencySymbol = ListUtils.SettingsSiteList.FirstOrDefault()?.CurrencySymbol ?? "$";
        ...
        private void SetData()
        {
            try
            {
                ...
                if (SongsClass != null)
                {
                    ...
                    if (SongsClass.Price.ToString(CultureInfo.InvariantCulture) == "0.00" || SongsClass.Price.ToString(CultureInfo.InvariantCulture) == "0")
                        PriceEditText.Text = GetText(Resource.String.Lbl_Free);
                    else
                        PriceEditText.Text = CurrencySymbol + SongsClass.Price.ToString(CultureInfo.InvariantCulture);
                    ...
                    IdPrice = SongsClass.Price.ToString(CultureInfo.InvariantCulture);
